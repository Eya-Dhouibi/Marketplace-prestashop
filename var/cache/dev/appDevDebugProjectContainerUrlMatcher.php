    ...
    public function match($rawPathinfo)
    {
        ...
        elseif (0 === strpos($pathinfo, '/modules')) {
            ...
            if (0 === strpos($pathinfo, '/modules/admin/cellule/toggle') && preg_match('#^/modules/admin/cellule/toggle/(?P<celluleId>[^/]++)/(?P<valid>[^/]++)$#sD', $pathinfo, $matches)) {
                $ret = $this->mergeDefaults(array_replace($matches, ['_route' => 'admin_cellule_toggle']), array (  '_controller' => 'Cellule\\Controller\\Admin\\AdminCelluleController::validateAction',));
                if (!in_array($canonicalMethod, ['GET', 'POST'])) {
                    $allow = array_merge($allow, ['GET', 'POST']);
